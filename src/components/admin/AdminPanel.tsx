...
import { List, ListItem, TextField, Button, MenuItem, Select } from '@mui/material';
...
const AdminDishesList = () => {
  ...
  const {
    refetch: updateDishesList,
  } = postService.useDishesQuery('');
  ...
  let SearchedPosts = [];
  const DeleteDish = (e: React.MouseEvent<HTMLButtonElement, MouseEvent>, itemId: string) => {
    e.preventDefault();
    ...
    <div className="flex-container flex-col items-center">
      <List className="list-group mb-4 sm:mb-0">
        ...
        {SearchedPosts.map((post) => (
          <div className="task list-group-item" key={post.id} item-id={post.id}>
            <img
              src={post.image}
              className="list-group-item"
              alt="Dish Image"
